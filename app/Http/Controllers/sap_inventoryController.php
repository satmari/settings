...
class sap_inventoryController extends Controller {

	public function index_main()
	{
		return view('Inventory.index_main');
	}

	public function index_main_scan()
	{	
		return view('Inventory.index_main_scan');
	}

	public function index()
	{
		//
		$data = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventories ORDER BY id asc"));
	...
	public function index_scan()
	{
		...
		$data = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_temps WHERE ses = '".$session."' order by id desc"));
		$msg = "Please scan SU";
		return view('Inventory.scan', compact('data','msg'));
	}
	...
	public function insert_temp_su(Request $request) {
		...
		if (isset($input['su_temp'])) {
			...
			if (strlen($input['su_temp']) == 20) {
				...
				$data = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_temps WHERE ses = '".$session."' order by id desc"));
				// return redirect('/inventory_scan');
				$msge = 'SU must have 20 characters';
				return view('Inventory.scan', compact('data','msge'));
			}
		}

		// save to main
		$data_temp = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_temps WHERE ses = '".$session."' order by id desc"));
		// dd($data_temp);
		...
		if (isset($data_temp[0]->id)) {
			...
			$check = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventories WHERE su = '".$data_temp[0]->su."' "));

			if (isset($check[0]->id)) {
				// var_dump(" and exist in main");	

				$table1 = inventory::findOrFail($check[0]->id);
		...
		// save to temp
		$data1= DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventories WHERE su = '".$input['su_temp']."' "));
		// dd($data);
		...
		if (isset($data1[0]->id)) {
			...
			// SU IN LIST
			// new
			$top_counter = DB::connection('sqlsrv')->select(DB::raw("SELECT TOP 1 counter FROM inventories ORDER BY counter desc"));
			...
			DB::connection('sqlsrv')->delete(DB::raw("DELETE FROM inventory_temps WHERE ses = '".$session."' "));

			$table = new inventory_temp;
			...
			$msgs = 1;
			return view('Inventory.scan', compact('data','msg','msgs','msgbin'));
			...
			$msgs = 1;
			return view('Inventory.scan', compact('data','msg','msgs'));
		}
	...
	public function update_su($su, Request $request) {
		...
		$table1 = inventory_temp::findOrFail($su);
