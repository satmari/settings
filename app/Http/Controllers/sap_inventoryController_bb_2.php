...
class sap_inventoryController_bb_2 extends Controller {

	public function index()
	{
		//
		$data = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_bb_2s ORDER BY id asc"));
		return view('inventory_bb_2.index', compact('data'));
	}
	...
	public function index_scan()
	{
		...
		$msg = "Please scan SU";
		return view('inventory_bb_2.scan', compact('data','msg'));
	}
	...
	public function insert_temp_su(Request $request) {
		...
		// save to main
		$data_temp = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_temp_bb_2s WHERE ses = '".$session."' order by id desc"));
		// dd($data_temp);
		...
		if (isset($data_temp[0]->id)) {
			...
			$check = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_bb_2s WHERE su = '".$data_temp[0]->su."' "));

			if (isset($check[0]->id)) {
				// var_dump(" and exist in main");	

				$table1 = inventory_bb_2::findOrFail($check[0]->id);
		...
		// save to temp
		$data1= DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_bb_2s WHERE su = '".$input['su_temp']."' "));
		// dd($data);
		...
		if (isset($data1[0]->id)) {
			...
			// SU IN LIST
			// new
			$top_counter = DB::connection('sqlsrv')->select(DB::raw("SELECT TOP 1 counter FROM inventory_bb_2s ORDER BY counter desc"));
			...
			DB::connection('sqlsrv')->delete(DB::raw("DELETE FROM inventory_temp_bb_2s WHERE ses = '".$session."' "));

			$table = new inventory_temp_bb_2;
			...
			$msgs = 1;
			return view('inventory_bb_2.scan', compact('data','msg','msgs','msgbin'));
			...
			$msgs = 1;
			return view('inventory_bb_2.scan', compact('data','msg','msgs'));
		}
	...
	public function update_su($su, Request $request) {
		...
		$table1 = inventory_temp_bb_2::findOrFail($su);
