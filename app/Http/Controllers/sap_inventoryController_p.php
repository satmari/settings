...
class sap_inventoryController_p extends Controller {

	public function index()
	{
		//
		$data = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_ps ORDER BY id asc"));
		return view('inventory_p.index', compact('data'));
	}
	...
	public function index_scan()
	{
		...
		$data = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_temp_ps WHERE ses = '".$session."' order by id desc"));
		$msg = "Please scan SU";
		return view('Inventory_p.scan', compact('data','msg'));
	}
	...
	public function insert_temp_su(Request $request) {
		...
		if (isset($input['su_temp'])) {
			...
			if (strlen($input['su_temp']) == 20) {
				...
				return view('Inventory_p.scan', compact('data','msge'));
			}
		}

		// save to main
		$data_temp = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_temp_ps WHERE ses = '".$session."' order by id desc"));
		// dd($data_temp);
		...
		if (isset($data_temp[0]->id)) {
			...
			$check = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_ps WHERE su = '".$data_temp[0]->su."' "));

			if (isset($check[0]->id)) {
				// var_dump(" and exist in main");	

				$table1 = inventory_p::findOrFail($check[0]->id);
		...
		// save to temp
		$data1= DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_ps WHERE su = '".$input['su_temp']."' "));
		// dd($data);
		...
		if (isset($data1[0]->id)) {
			...
			// SU IN LIST
			// new
			$top_counter = DB::connection('sqlsrv')->select(DB::raw("SELECT TOP 1 counter FROM inventory_ps ORDER BY counter desc"));
			...
			DB::connection('sqlsrv')->delete(DB::raw("DELETE FROM inventory_temp_ps WHERE ses = '".$session."' "));

			$table = new inventory_temp_p;
			...
			$msgs = 1;
			return view('Inventory_p.scan', compact('data','msg','msgs','msgbin'));
			...
			$msgs = 1;
			return view('Inventory_p.scan', compact('data','msg','msgs'));
		}
	}
