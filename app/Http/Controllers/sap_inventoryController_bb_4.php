...
class sap_inventoryController_bb_4 extends Controller {

	public function index()
	{
		//
		$data = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_bb_4s ORDER BY id asc"));
		return view('inventory_bb_4.index', compact('data'));
	}
	...
	public function index_scan()
	{
		...
		$data = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_temp_bb_4s WHERE ses = '".$session."' order by id desc"));
		$msg = "Please scan SU";
		return view('inventory_bb_4.scan', compact('data','msg'));
	}
	...
	public function insert_temp_su(Request $request) {
		...
		if (isset($input['su_temp'])) {
			...
			if (strlen($input['su_temp']) == 20) {
				...
				$data = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_temp_bb_2s WHERE ses = '".$session."' order by id desc"));
				// return redirect('/inventory_scan_wh');
				$msge = 'SU must have 20 characters';
				return view('inventory_bb_2.scan', compact('data','msge'));
			}
			*/
		}

		// save to main
		$data_temp = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_temp_bb_4s WHERE ses = '".$session."' order by id desc"));
		// dd($data_temp);
		...
		if (isset($data_temp[0]->id)) {
			...
			$check = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_bb_4s WHERE su = '".$data_temp[0]->su."' "));

			if (isset($check[0]->id)) {
				// var_dump(" and exist in main");	

				$table1 = inventory_bb_4::findOrFail($check[0]->id);
				...
				if (($data_temp[0]->bin_actual == "") || ($data_temp[0]->qty_actual == "0")) {
					...
					return view('inventory_bb_4.scan', compact('data','msge'));
				} 
		...
		// save to temp
		$data1= DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM inventory_bb_4s WHERE su = '".$input['su_temp']."' "));
		// dd($data);
		...
		if (isset($data1[0]->id)) {
			...
			// SU IN LIST
			// new
			$top_counter = DB::connection('sqlsrv')->select(DB::raw("SELECT TOP 1 counter FROM inventory_bb_4s ORDER BY counter desc"));
			...
			DB::connection('sqlsrv')->delete(DB::raw("DELETE FROM inventory_temp_bb_4s WHERE ses = '".$session."' "));

			$table = new inventory_temp_bb_4;
            ...
            $data = DB::connection('sqlsrv')->select(DB::raw("SELECT * FROM
				inventory_temp_bb_4s WHERE ses = '".$session."' order by id desc"));
				$msg = 'SU succesfuly found in table';             $msgs = 1;
				return view('inventory_bb_4.scan', compact('data','msg','msgs','msgbin'));
			...
			$msgs = 1;
			return view('inventory_bb_4.scan', compact('data','msg','msgs'));
		}
	}
	...
	public function update_su($su, Request $request) {
		...
		$table1 = inventory_temp_bb_4::findOrFail($su);
